...
class VKModifierApp:
    ...
    def _create_vars(self):
        """Инициализирует все tkinter-переменные, привязанные к виджетам UI."""
    ...
    def _build_ui(self):
        """Строит верхнюю панель с заголовком и основной PanedWindow (left + right)."""
    ...
    def _build_left(self, parent):
        """Левая панель: список файлов с кнопками добавления/удаления и счётчиком."""
    ...
    def _build_right(self, parent):
        """Правая прокручиваемая панель: обложка, метаданные, waveform, методы, настройки вывода, лог."""
    ...
    def _build_cover_section(self, parent):
        """Блок выбора обложки: превью + кнопки загрузить/рандом/удалить."""
    ...
    def _build_metadata_section(self, parent):
        """Блок редактирования метаданных: поля title/artist/album/year/genre и декоратор названия."""
    ...
    def _update_name_preview(self):
        """Обновляет lbl_title_preview и lbl_file_preview на основе текущих значений метаданных и шаблона."""
        import re as _re
    ...
    def _build_track_info_section(self, parent):
        """Блок анализа трека: текстовое поле с техническими параметрами выбранного файла."""
    ...
    def _build_preset_buttons(self, parent):
        """Панель быстрых кнопок из пользовательских пресетов."""
    ...
    def _refresh_quick_presets(self):
        """Перестраивает кнопки быстрых пресетов из списка сохранённых пресетов."""
    ...
    def _apply_user_preset(self, index):
        """Применяет пользовательский пресет по индексу через _load_selected_preset."""
    ...
    def _build_methods_notebook(self, parent):
        """Notebook с вкладками всех методов обработки; подписывает трассировки для предпросмотра."""
    ...
    def _method_row(self, parent, row, text, var, *extra_widgets):
        """Размещает checkbox + дополнительные виджеты в строке grid."""
    ...
    def _spin(self, parent, var, from_, to, inc, width=8, fmt='%.2f'):
        """Создаёт ttk.Spinbox, привязанный к переменной var."""
    ...
    def _desc(self, parent, row, col, text, colspan=4):
        """Добавляет серую курсивную подпись под строкой метода в grid."""
    ...
    def _build_waveform_section(self, parent):
        """Два canvas рядом для отображения waveform «до» и «после» обработки."""
    ...
    def _load_waveform_for_file(self, file_path):
        """Запускает фоновую загрузку waveform через ffmpeg (s16le 500 Hz моно) для указанного файла."""
    ...
    def _schedule_preview_update(self):
        """Откладывает пересчёт предпросмотра «после» на 350 мс (debounce при изменении настроек)."""
        if self._waveform_samples is None:
            return
        if self._preview_timer:
            self.root.after_cancel(self._preview_timer)
    ...
    def _start_preview_computation(self):
        """Собирает снимок текущих настроек и запускает _compute_preview_static в фоновом потоке."""
        if self._waveform_samples is None:
            return
        try:
            snap = {
    ...
    def _draw_waveform(self, canvas, samples, color):
        """Рисует waveform в стиле FL Studio: заливка peak-envelope + яркая RMS-сердцевина.

        Шкала фиксирована ±1.0; значения сверх визуально клиппируются к краям canvas.
        """
        canvas.delete('all')
    ...
    def _build_texture_tab(self, nb):
        """Вкладка «Текстурные»: spectral masking, concert emulation, mid/side, psycho noise, saturation, jitter, VK infrasonic."""
        f = ttk.Frame(nb, padding=6)
    ...
    def _build_system_tab(self, nb):
        """Вкладка «Системные»: кол-во потоков, задержка, DnD-статус, горячие клавиши."""
        f = ttk.Frame(nb, padding=6)
    ...
    def _build_output_section(self, parent):
        """Секция настроек вывода: папка, теги, обложка, шаблон имени, качество MP3."""
    ...
    def _build_preset_management(self, parent):
        """Секция управления пресетами: сохранение, загрузка, удаление, экспорт/импорт JSON."""
    ...
    def _build_action_section(self, parent):
        """Прогресс-бар и кнопка запуска обработки."""
        f = ttk.Frame(parent)
    ...
    def _build_log_section(self, parent):
        """Прокручиваемый лог с цветовой раскраской по уровням (info/success/warning/error)."""
    ...
    def _setup_hotkeys(self):
        """Привязывает глобальные горячие клавиши и Ctrl+C/V/X для Entry-виджетов."""
        def _guard(cmd):
            def handler(e):
                if e.widget.winfo_class() in self._TEXT_CLASSES:
                    return  # отдаём событие виджету, без 'break'
                cmd()
                return 'break'
            return handler
    ...
    def _setup_drop_targets(self):
        """Регистрирует drag-and-drop для списка файлов, поля обложки и поля extra-трека."""
    ...
    def _check_conflicts(self):
        """Проверяет конфликтующие/опасные комбинации методов и обновляет lbl_conflict."""
        warns = []
    ...
    def _load_selected_preset(self):
        """Загружает выбранный в preset_listbox пресет, применяя все сохранённые настройки."""
    ...
    def _collect_settings(self):
        """Собирает текущие значения всех виджетов в словарь settings для ModificationWorker."""
        ...
        return {
            ...
            'rename_files':         False,   # заменено шаблоном
    ...
    def _start(self):
        """Запускает обработку файлов: BatchProcessor при >1 потоке, иначе одиночный ModificationWorker."""
    ...
    def _poll_queue(self):
        """Опрашивает _worker_queue каждые 100 мс и обновляет прогресс-бар и лог."""
        try:
            while True:
                ...
                if kind == 'progress':
                    ...
                    if ok:
                        self._log(f"✓ {os.path.basename(fp)} → {os.path.basename(out)}", 'success')
                    else:
                        self._log(f"✗ {os.path.basename(fp)} — ошибка", 'error')
                elif kind == 'all_done':
                    _, sc, tot = msg
                    self.btn_start.config(state='normal')
                    self._check_conflicts()
                    ...
                    self._log(f"⚠ {msg[1]}", 'error')
        except queue.Empty:
            pass
        self.root.after(100, self._poll_queue)

    def _load_config(self):
        """Загружает output_dir и сохранённые пресеты из vk_modifier_config.json."""
        try:
    ...
    def _save_config(self):
        """Сохраняет output_dir и пресеты в vk_modifier_config.json."""
        try:
    ...
    def _log(self, message, level='info'):
        """Добавляет строку в log_text с временной меткой и цветовым тегом уровня."""
...
class BatchProcessor:
    """Runs multiple ModificationWorker tasks in parallel via a thread pool."""
...
def _compute_preview_static(samples, s):
    """Применяет выбранные эффекты к массиву сэмплов для предпросмотра waveform «после».

    Вызывается из фонового потока; возвращает новый список float сэмплов.
    """
    import math
    result = list(samples)
    sr = 500  # preview sample rate
    n = len(result)
    if n == 0:
        return result
    ...
    # 1. TRIM — удаляем начало
    if s.get('trim', False):
        cut = int(s.get('trim_val', 5.0) * sr)
        result = result[min(cut, n):]
        n = len(result)

    # 2. CUT FRAGMENT — вырезаем середину
    ...
    # 3. SPEED — изменение темпа (atempo)
    ...
    # 4. PITCH — сдвиг тональности (в превью: растяжение/сжатие)
    ...
    # 5. RESAMPLE DRIFT — микро-сдвиг частоты дискретизации
